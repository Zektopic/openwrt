...
## 2024-05-24 - Buffer Overflow Prevention in nvram.c
**Vulnerability:** In `package/utils/nvram/src/nvram.c`, the `_nvram_realloc` function used unbounded `strcpy` to copy strings into dynamically allocated structures. While the allocation sizes were based on `strlen(name) + 1` and `strlen(value) + 1`, a discrepancy between evaluation and usage, or potential integer overflows during size calculation, could theoretically lead to heap buffer overflows. Furthermore, casting pointers to `int` for alignment checks (`(int)ptr % 4`) caused compiler warnings on 64-bit platforms and could lead to incorrect alignment logic if the pointer value exceeded the bounds of an `int`.
**Learning:** Legacy string manipulation functions (`strcpy`) are unsafe and easily introduce memory corruption bugs. Unchecked arithmetic on string lengths before allocation can lead to integer overflows, resulting in under-sized buffers. Casting pointers to `int` is non-portable and unsafe on 64-bit architectures.
**Prevention:** Always use safe bounded string copy mechanisms like `strlcpy`. Implement explicit `SIZE_MAX` overflow checks when calculating allocation sizes, especially for user-controlled or external input. Use variables to cache the calculated `strlen` for both memory allocation size and subsequent copy operations to guarantee safety and consistency. For pointer arithmetic or alignment checks, use `uintptr_t` to ensure 64-bit compatibility.
## 2024-05-01 - Prevent Command Injection in ucode fs.popen
...
**Prevention:** Prefer using `memcpy(dst, src, size)` instead of `strncpy` for these cases to avoid linter warnings and security scanner rejections, or explicitly use `strncpy` with the mathematically remaining destination size `int remaining = size - (val - dest_start);`.
## 2024-07-03 - Command Injection in scripts/download.pl
...
## 2025-05-28 - Prevent Option Injection in system() pipes
...
## 2024-07-04 - Command Injection in scripts/download.pl (Part 2)
**Vulnerability:** Command injection exists in `scripts/download.pl` due to unsanitized variables (like `$url`, `$filename`, `$mirror`) being directly interpolated into strings executed by the shell via `open(..., '-|', @cmd)` or backticks. If an attacker controls the download URL or filename, they can inject shell metacharacters and execute arbitrary commands.
**Learning:** In Perl scripts that invoke system commands dynamically, when array-based `system(@args)` cannot be used (e.g., when building shell pipelines for `aria2c`), dynamic variables cannot be safely enclosed in single quotes directly because the variables themselves might contain shell breakouts (like `'; id; echo '`).
**Prevention:** Introduce a `shell_quote()` helper function that securely escapes single quotes (by replacing `'` with `'\''`) and wraps the entire resulting string in single quotes (e.g., `return "'$str'";`), and meticulously apply this function to every dynamic variable before interpolating it into a shell command string.

## 2024-05-24 - Option Injection via `cat |` in shell commands
...
## 2024-07-20 - Missing Global Flag in Shell Escape
**Vulnerability:** Shell character escaping (replacing single quotes) in Perl without the global `/g` flag.
**Learning:** In Perl, omitting the `/g` flag in a regex substitution (`s/'/'\''/`) only escapes the first instance of the character, allowing command injection if multiple single quotes are provided in user input interpolated into a shell command.
**Prevention:** Always use the `/g` flag when escaping shell characters in Perl to ensure all instances are replaced.
