...
## 2024-07-22 - Combine regex blacklist checking to avoid O(N*M) loop overhead
**Learning:** Checking a large string list against multiple regular expressions iteratively inside a nested loop causes significant overhead in Python. The C regex engine natively optimizes OR-ed patterns.
**Action:** Combine the list of regexes into a single combined regex using `"|".join(f"(?:{regex.pattern})"...)` before the loop, pushing the workload entirely into the C regex matching logic. Ensure dynamic arguments are processed first before compiling the combined regex.
...
## 2026-10-31 - [Python json.loads(path.read_text()) overhead]
**Learning:** In standard CPython, `json.load(f)` does not stream the file descriptor chunk-by-chunk for memory savings. Under the hood, it calls `json.loads(f.read())` and loads the entire string into memory before parsing. However, replacing `json.loads(path.read_text())` with `with open(path, 'r') as f: json.load(f)` avoids explicitly constructing the intermediate string on the Python heap before passing it to the C-level json parser, resulting in a slightly faster and more idiomatic execution, even if it doesn't prevent loading the entire payload into memory overall.
**Action:** Replace `json.loads(path.read_text())` with `with open(path, 'r') as f: json.load(f)` when parsing json files for idiomatic best practice and minor performance improvements.
