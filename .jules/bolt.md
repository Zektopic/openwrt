...
## 2026-05-11 - [Python `shutil.copyfileobj` and chunking to avoid O(N) memory]
**Learning:** In Python firmware packing/unpacking scripts (like `scripts/belkin-header.py` and `scripts/tplink-mkimage-2022.py`), reading the entire input file or a large partition into a variable using `f.read()` or `f.read(size)` before writing causes severe O(N) memory bloat, scaling linearly with image size.
**Action:** When calculating checksums and generating headers, read the input in small 64KB chunks in a loop (`while True: chunk = f.read(65536)`). To output data directly, either stream the remaining chunks manually tracking remaining size (`while bytes_left > 0: chunk = f.read(min(65536, bytes_left))`), or use `shutil.copyfileobj(in_f, out_f)` after seeking to the appropriate start point. This ensures memory usage remains constant O(1).
