...
## 2024-11-20 - [Optimize large JSON file parsing in scripts]
**Learning:** `json.loads(file.read())` creates a massive intermediate string in memory before parsing, causing significant overhead when parsing large JSON files like SBOM or index files.
**Action:** Replace `json.loads(file.read())` with `json.load(file)` to stream the JSON parsing directly from the file descriptor, avoiding large intermediate strings.
